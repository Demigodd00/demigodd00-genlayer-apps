...
export async function discoverWalletProviders(
  ...
  const injected = host.ethereum?.providers?.length
    ? host.ethereum.providers
    : [host.ethereum];
  injected.forEach((provider, index) =>
    add(
      provider,
      `legacy:${index}`,
      provider ? providerName(provider) : "Browser wallet",
    ),
  );
...
export async function connectStudioWallet(
  provider: EthereumProvider | undefined,
): Promise<`0x${string}`> {

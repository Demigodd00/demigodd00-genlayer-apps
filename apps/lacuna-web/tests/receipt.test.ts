    ...
    leader_receipt: [
      {
        ...
        result: {
          status: "rollback",
          payload: "[EXPECTED] bond is not active",
        },
      },
    ],
  },
...
test("a finalized contract rejection exposes the contract reason", () => {
  assert.throws(
    () => assertSuccessfulExecution(rejected),
    /bond is not active/,
  );
});
...
test("finality without execution evidence is never treated as success", () => {
  assert.throws(
    () => assertSuccessfulExecution({ status: 7, result: 6 }),
    /did not confirm successful execution/,
  );
  assert.throws(
    () => assertSuccessfulExecution(null),
    /did not confirm successful execution/,
  );
});
    ...
    () =>
      assertSuccessfulExecution({
        consensus_data: {
          leader_receipt: {
            execution_result: "SUCCESS",
            result: { status: "rollback", payload: "rolled back" },
          },
        },
      }),
    /rolled back/,
    ...
    () =>
      assertSuccessfulExecution({
        consensus_data: {
          leader_receipt: {
            execution_result: "SUCCESS",
            genvm_result: { error_description: "VM failed" },
          },
        },
      }),
    /VM failed/,

  ...
  const provider: EthereumProvider = {
    request: async (request) => {
      ...
      switch (request.method) {
        ...
        case "eth_accounts":
          return options.currentAccounts ?? [account];
        case "eth_chainId":
          return chain;
...
test("a wallet already on StudioNet connects without a Snap", async () => {
  ...
  assert.deepEqual(
    wallet.calls.map((call) => call.method),
    ["eth_requestAccounts", "eth_chainId", "eth_chainId", "eth_accounts"],
  );
});
...
test("wallet rejection and ignored switching fail closed", async () => {
  ...
  assert.equal(
    rejected.calls.some((call) => call.method === "wallet_addEthereumChain"),
    false,
  );
  await assert.rejects(
    connectStudioWallet(
      mockWallet({ chain: "0x1", ignoreSwitch: true }).provider,
    ),
    /Switch your wallet to StudioNet/,
  );
});

test("the current account is re-read after network approval", async () => {
  assert.equal(
    await connectStudioWallet(
      mockWallet({ chain: "0x1", currentAccounts: [otherAccount] }).provider,
    ),
    otherAccount,
  );
});
...
test("missing wallets and invalid accounts fail clearly", async () => {
  await assert.rejects(
    connectStudioWallet(undefined),
    /No compatible browser wallet/,
  );
  for (const accounts of [[], ["invalid"], [`0x${"0".repeat(40)}`]]) {
    await assert.rejects(
      connectStudioWallet(mockWallet({ accounts }).provider),
      /invalid account/,
    );
  }
});
...
test("EIP-6963 announcements are discovered and deduplicated against legacy injection", async () => {
  ...
  const host = {
    ...
    removeEventListener: (_type: string, next: EventListener) => {

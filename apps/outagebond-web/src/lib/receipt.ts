...
/** A finalized GenLayer transaction can still contain a failed execution. */
export function assertSuccessfulExecution(receipt: unknown): void {
  ...
  const failed = Boolean(tx.error || vm.error_code || vm.error_description)
    || result.status === "rollback" || result.status === "error";
  if (!failed && execution === "SUCCESS") return;
  if (!failed && execution === undefined && tx.txExecutionResultName === "FINISHED_WITH_RETURN") return;
  ...
  throw new Error(typeof reason === "string" ? reason : "GenLayer did not confirm successful execution. Check the transaction before retrying.");
}

...
The named test service is **OutageBond Synthetic API**. Both source reports are openly labeled synthetic and controlled by the demo author. They use separate public hosts, but do not demonstrate independent organizations or a real operational outage. The first eligible-case attempt was inconclusive because the initial fixture wording was ambiguous; the fixture was clarified and the normal permissionless retry path used. The acceptance history retains that earlier reading rather than presenting every attempt as successful.

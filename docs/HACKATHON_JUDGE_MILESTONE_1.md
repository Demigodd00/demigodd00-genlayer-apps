...
Release checks: 95 direct contract/RPC tests (including 48 milestone tests), 20 frontend tests, contract lint, frontend typecheck/lint/build, and one independent live readback test passed. The live test recomputes digests, citation excerpts and totals and verifies actual transaction receipts rather than trusting the journal's success labels. The 60-second video was fully decoded without errors. Public website publication is the final release step.
